...
function createLoggedWrapperFunction (functionMeta) {
    ...
    var functionObj = function (args) {
        ...
        if (args.session.replay) {
            ...
            moduleCallId = args.session.moduleCallId = getModuleCallId(functionMeta, args)
            ...
            // log call
            moduleCallId = args.session.moduleCallId = logCall(functionMeta, args)
        }
...
/**
 * @function getModuleCallId
 *
 * @param {object} functionMeta - function meta data
 * @param {object} args - original caller args
 */
function getModuleCallId (functionMeta, args) {
    try {
        // session must be passed as args or as param to args
        var session = args.session
        // get args data
        var argsId = stableIdWithData(args)
        // build call data
        var moduleCallData = {
        ...
        // get id
        var moduleCallId = stableId(moduleCallData)
        // return module call id to retrieve stored result
        return moduleCallId
    }
    catch (err) {
        // log errors
...
function logCall (functionMeta, args) {
    try {
        ...
        // log args data
        log.data(argsId)
        /* disable stack logging for now
         * stack is dodgy and needs to be reworked or removed
        // stack id if any
        var stackId = stableIdWithData(session.stack)
        // log data
        log.data(stackId)
        */
        // build call data
        ...
        // add call time after creating id so that id can be replayed
        moduleCallData.moduleCallCreateTime = microTimestamp()
        // log module call
...
function logReturn (moduleCallId, args, ret) {
    ...
    // try to inject logger into promise chain
    try {
...
function runAfterLogged (functionMeta, args, originalRet, moduleCallId) {
    ...
    return originalRet.then(function (res) {
        ...
        for (var i=0; i < functionMeta.after.length; i++) {
            ...
            var promise = (functionObj)({args: args, res: res, session: session})
